    ...
    Behavior<ListView>
{
    ...
    {
        if ((Keyboard.Modifiers & (ModifierKeys.Control | ModifierKeys.Shift | ModifierKeys.Alt)) == ModifierKeys.None && VisualTreeHelper.HitTest(AssociatedObject, e.GetPosition(AssociatedObject)).VisualHit is { } visualHit && visualHit.GetVisualAncestor<ListViewItem, GridViewColumnHeader, ScrollBar>() is null)
