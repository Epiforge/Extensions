    ...
    SelectorDataVirtualization<ComboBox>
{
    ...
    {
        if (AssociatedObject is { } comboBox && comboBox.SelectedItem is IDataVirtualizationItem selectedItem)
    ...
    {
        if (this.scrollViewer is null && AssociatedObject is ComboBox comboBox && comboBox.Template?.FindName("PART_Popup", comboBox) is Popup popup && popup.Child.GetVisualDescendent<ScrollViewer>() is { } scrollViewer)
